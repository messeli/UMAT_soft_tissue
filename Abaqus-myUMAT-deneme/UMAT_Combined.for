...
C       INTEGER NTENS,NDI,NSTATV,NPROPS,MATERIALNUMBER
   ...
   30   CONTINUE     
        ...
        CALL KMYINVER(DFGRD,DFGRDI) !|Inverse deformation gradient
...
C    +          +MATMUL(MATMUL(CMATB,XA),TRANSPOSE(XA)) !|CB symmetric
        ...
        S1B = 2.D0*DW1CB
        CALL KMYDEVCUR(CMAT,S1B,DEVS1B)
        S1 = XJ_23*DEVS1B
        ...
        S2B = 2.D0*DW2CB
        CALL KMYDEVCUR(CMAT,S2B,DEVS2B)
        S2 = XJ_23*DEVS2B
...
C         DOUBLE PRECISION :: X,Y
      ...
      SUBROUTINE KMYDEVCUR(CMAT,AB,DEV_AB) !|B for Bar.
        ...
        CALL KMYTRACE(MATMUL(TRANSPOSE(AB),CMAT),TR_)
        CALL KMYINVER(CMAT,CMATI)
